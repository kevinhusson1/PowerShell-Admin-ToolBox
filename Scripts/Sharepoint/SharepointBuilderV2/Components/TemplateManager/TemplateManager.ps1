...
function Register-TemplateEvents {
    ...
    if ($Ctrl.CbFolderTemplates) {
        ...
        $Ctrl.CbFolderTemplates.Add_SelectionChanged({
                ...
                # Gestion de la visibilité du conteneur (NOUVEAU)
                $formScroll = $Window.FindName("DynamicFormScrollViewer")
                if ($formScroll) { $formScroll.Visibility = [System.Windows.Visibility]::Visible }
                
                # S'assurer que le panel est visible aussi (au cas où)
                if ($Ctrl.PanelForm) { $Ctrl.PanelForm.Visibility = [System.Windows.Visibility]::Visible }
                ...
                try {
                    ...
                    foreach ($elem in $layout) {
                        ...
                        elseif ($elem.Type -eq "ComboBox" -or $elem.Type -eq "Choix") {
                            $c = New-Object System.Windows.Controls.ComboBox -Property @{ 
                                Name        = "Input_$($elem.Name)"
                                ItemsSource = $elem.Options
                                Width       = if ($elem.Width) { [double]$elem.Width } else { 120 }
                            ...
                            if ($defaultValue -and $elem.Options -contains $defaultValue) { $c.SelectedItem = $defaultValue }
                                ...
                                # Sélectionne le premier élément si aucune valeur par défaut n'est spécifiée ou trouvée
