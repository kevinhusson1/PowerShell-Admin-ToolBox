...
foreach ($fichier in $fichiers) {
    ...
    $cheminRelatif = $fichier.DirectoryName.Substring($source.Length)
    if ($cheminRelatif.StartsWith("\")) { $cheminRelatif = $cheminRelatif.Substring(1) }
    
    # Dossier de destination respectif
    $dossierDestination = Join-Path $destination $cheminRelatif
    
    # Créer le dossier de destination s'il n'existe pas
    if (!(Test-Path $dossierDestination)) {
    ...
    $cheminDestination = Join-Path $dossierDestination $nouveauNom

    # Si le fichier existe déjà, ajouter un compteur
    $compteur = 1
    while (Test-Path $cheminDestination) {
    ...
    Write-Host "Copié: $($fichier.Name) -> $nouveauNom (dans $dossierDestination)"
}
